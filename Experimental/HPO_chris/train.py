...
def chris(HP, train_dataloader, validation_dataloader, test_dataloader, cuda_device):
    ...
    root_path = Path(__file__).parent.absolute().joinpath('experimental')
    ...
    split_train, split_test = round(len(experimental_dataloader.dataset)*0.8), round(len(experimental_dataloader.dataset))-round(len(experimental_dataloader.dataset)*0.8)

    train_exp, test_exp = torch.utils.data.random_split(experimental_dataloader.dataset, (split_train, split_test))
    ...
    if validation_dataloader != None:
        test(validation_dataloader, 'Validation Test', disp_CM=False, Net = Net, Tensor=Tensor)
    true_list, pred_list, f_score = test(experimental_dataloader_test,
                                'Experimental Test', disp_CM=True, Net = Net, Tensor=Tensor)
    # showFailures(true_list, pred_list, experimental_dataloader_test)
...
path = Path(__file__).parent.absolute().joinpath('experimental')
print(path)
# add in cuda device
