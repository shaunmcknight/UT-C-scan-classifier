...
split_val, split_test = round(len(experimental_dataloader.dataset)/2), round(len(experimental_dataloader.dataset))-round(len(experimental_dataloader.dataset)/2)
valid_exp, test_exp = torch.utils.data.random_split(experimental_dataloader.dataset, (split_val, split_test))

experimental_dataloader_val = DataLoader(
    valid_exp,
    batch_size=16,
    shuffle=True,
    # num_workers=1,
...
def train(
    ...
    for epoch in range(hp.epoch, n_epochs):
        ...
        if (np.mean(loss.item()*hp.batch_size)) < 5:
            break
...
for i in range(100):
    ...
    test()
    precision, recall, f_score, accuracy = testExp(experimental_dataloader)
